...
###Loading parametres:from Bio.Seq import Seq
path_main=str(os.getcwd())
param_spe=list()
with open(path_main+"/LoRTE-Parameters",'r') as param:
...
log=open(path_main+'/log-LorTE','w')
log.close()
log=open(path_main+'/log-LorTE','a')
...
def MEGABLAST(reads,query,eEevalue):
	...
	restOfCommand=" -num_threads 2 -outfmt \"7 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore\" -out "+resultsLocation+queryName+"-megablast"
	os.system("blastn -task megablast"+readscommand+querycommand+evaluePart+restOfCommand)
...
def BLAST(nameFileSequenceExtracted,consensus,eeValue):
	...
	C2=" -evalue "+eeValue+" -db "+consensus+" -num_threads 2 -outfmt \"7 sseqid sstart send length qseqid  evalue\" -max_target_seqs 1 -out "+nameFileSequenceExtracted+"-blastn"
	blast=C1+C2
	os.system(blast) 
...
def BLAST2(nameFileSequenceExtracted,consensus,eeValue):
	...
	"""
	C1="blastn -task blastn -query "+nameFileSequenceExtracted	
	C2=" -evalue "+eeValue+" -db "+consensus+" -num_threads 2 -outfmt \"7 sseqid qseqid sstart send length evalue\" -out "+nameFileSequenceExtracted+"-blastn"
	blast=C1+C2
	os.system(blast) 
	return(nameFileSequenceExtracted+"-blastn")
...
def SimpleBlast(reads,query):
	...
	restCommand=" -num_threads 2 -outfmt \"7 qseqid sseqid pident length mismatch gapopen qstart qend sstart send evalue bitscore\" -out "+PathResults+nomquery+"-blastN"
...
def CreateSumUp(The53,The5,The3,The053,The05,The03,listOfTransposons,outputName,unresolved,StepProgram):
	...
	listResults=list()
	listResults.append("#Name of TE\tPositiveHit-TE\tNegativeHit(Tot)\tNegativeHit-TE<50nt\tNegativeHit-TE>50nt\tResults")
	listPositive=list()
	listMegablast=list()
	...
	for x in listTeName:
		...
		if len(x)>0:
			...
			listResults.append(nom+'\t'+str(numberHitPositif)+'\t'+str(numberHitNegative)+'\t'+str(Small)+'\t'+str(Big)+'\t'+bilanprov)
			i+=1
		ii=(i*100)/tailleF
		sys.stdout.write("\r%d%%" % ii)
		sys.stdout.flush()
	...
	fileSumUp.write("Pourcentage:\tAbsent\t%s\tAmbiguous\t%s\tPresent\t%s\tNon sequenced\t%s\tPolymorph\t%s\tUnsolvable\t%s\n\n" %((str(AB)),(str(AM)),(str(P)),(str(W)),(str(PO)),(str(UN))))
	for c in listResults:
		fileSumUp.write(c+'\n')
	fileSumUp.close()
	
	return(outputFileNameShort,outputFileNameSumUp,outputFileNamePoly)
...
def CreateFinalSumup(sortieDiff,SumUpFile,StepNumber):
	"""
	...
	if StepNumber==1:
		with open(SumUpFile) as listSumUp:
			listSumUp=listSumUp.read().split('\n')
		lineSumUp=listSumUp[0]
		lineSumUp=lineSumUp.split('\t')
		a=float()
...
(smallNegative,sumUpStepOne,polymorph1)=CreateSumUp(StepOne53,StepOne5,StepOne3,StepOne053,StepOne05,StepOne03,path_TE_annotated,nameOutputSumupOne,NonResolvable,1)
...
smallNegativeOccurence=CreateFinalSumup(smallNegativeSequences,sumUpStepOne,1)
...
(newInsertions,FileSumUpTwo,polymorph2)=CreateSumUp(C253,C25,C23,C2053,C205,C203,ListTeSecondStep,nameOutputSumUpTwo,NonResolvable2,2)
...
finalite2=CreateFinalSumup(zoooo,FileSumUpTwo,2)
...
os.system("mv "+poly2file+" "+my_summary_file)
