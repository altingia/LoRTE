...
parser = argparse.ArgumentParser(description='transposons detection')
parser.add_argument('-r', '--reference', help='Complete path to the Reference Genome', required=True)
parser.add_argument('-L', '--telist', help='Complete path to the TE list', required=True)
parser.add_argument('-i', '--input', help='Complete path to the Reads', required=True)
parser.add_argument('-c', '--consensus', help='Complete path of the TE consensus', required=True)
parser.add_argument('-o', '--output', default=os.getcwd()+'/output',help='Name of results folder, recommend absolute path', required=False)
parser.add_argument('-e', '--evalue', default=1e-40,help='E-vaLue for BLASTN & MEGABLAST', required=False)
parser.add_argument('-d', '--depth', default=10,help='Read coverage of your reads', required=False)
parser.add_argument('-f', '--format', default="4,1,2,3,5,'t',0",help='Format of the TE list:  nameTE,nameContig,SStart,SStop,Sens(\'XXXXX\'if not defined), separator_col, first line of data(line 0 can be descriptive of a blast output) ex:0,3,2,4,"XXXXX","\t",0 for a BLAST output file in which the order would be: nameTE, qstart, qend, Subject, Sstart, Sstop separated by tabulation, orientation is not precised, start with no description line)', required=False)
parser.add_argument('-w', '--window', default=10000,help='Maximum window size between two flanking sequences', required=False)
parser.add_argument('-l', '--length', default=200,help='Length of the flanking sequences that will be collected', required=False)
parser.add_argument('-n', '--numthread', default=2, type=int, help='Number of CPU to be used', required=False)
args = parser.parse_args()

#Localisation of reference genome:
path_ref_genome = str(args.reference)
#Localisation of the Transposable Element identified on the reference genome:
path_TE_annotated = str(args.telist)
#Localisation of Long read file (Pac bio):
path_pacbio = str(args.input)
#Consensus of transposable elements:
path_consensus_TE = str(args.consensus)
...
#create log file
log=open(name_folder_results+LogName,'w')
log.close()
log=open(name_folder_results+LogName,'a')
...
log.write('Creating file of UnresolvableStep1 file.'+"\n")
...
log.write('Alignment of flanking sequence on Reads'+"\n")
start = time.time()
